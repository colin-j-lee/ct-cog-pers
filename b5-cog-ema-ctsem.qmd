...
setwd('~/Dropbox/Academia/Projects/CTSEMB5StatesCog') #this is for EW's machine
#setwd('<Colin's Pathstring Goes Here') 
...
ema_df = ema_df |> 
  rename(DateTime=Date) |> #First, make the column more informative
  select(-c(A:N_emoVol)) |> #these are averages of observed values, but we should impute first and then form the averages. 
  group_by(SID) |> 
  mutate(StartDateTime = min(DateTime)) |>  #get the start date for each participant
  ungroup() |> 
  group_by(SID,Day) |> 
