...
---
EW: Emorie provided this excerpt for how she cleaned the outliers:
...
To do: what to do about within-person centering for quantity

Done: within-person center level 1 variables
...
sc_soccog <- sc_soccog %>% 
  ...
  mutate(
    ...
    , lonely_z = zscore(lonely)
    , lonely_z_lag = lag(lonely_z)
...
sc_soccog <- sc_soccog %>% 
  ...
  unnest(data) #%>%
  # group_by(SID) %>%
  # mutate(lonely_mssd = rmssd(lonely)) %>% 
  # ungroup()
...
#     lonely_ct_rx
#   ) %>%
#   select(lonely_mssd, Estimate) %>%
#   cor()
# 
# sc_soccog %>%
#   select(SID, lonely_mssd) %>% 
#   distinct() %>% arrange(desc(lonely_mssd))
```


```{r, eval = F}
lonely_ar_m <- brm(
  lonely_z ~ -1 + lonely_z_lag + (-1 + lonely_z_lag | SID)
  , data = sc_soccog
...
lonely_ar_rx <- rx_fun(lonely_ar_m) 

cat_plot(lonely_ar_rx, term = "lonely_z_lag")
rf_dist(lonely_ar_rx)

sc_soccog <- sc_soccog %>%
  left_join(
    lonely_ar_rx %>%
      select(SID, lonely_ar = Estimate) %>%
...
lonely_ar_rx %>%
  ...
  geom_density(fill = "#9AA0F9", alpha = .75) + 
  annotate("segment", arrow = arrow(type = "closed", length=unit(2, "mm")), size = 1, x = .2, xend = 0, y = -.02, yend = -.02) + 
  annotate("text", label = "Low Instability", x = .55, y = -.2, fontface = 2) + 
  annotate("segment", arrow = arrow(type = "closed", length=unit(2, "mm")), size = 1, x = .2, xend = .75, y = -.02, yend = -.02) + 
  annotate("text", label = "High Instability", x = .08, y = -.2, fontface = 2) + 
  labs(
    title = "Individual Differences in\nLoneliness Instability"
    , y = ""
    , x = "Loneliness Instability"
...
rescale_time <- function(x){
  x %>%
    ...
    mutate(
      ...
      # , lonely_z = jitter_fun(lonely_z)
      ) %>%
    as.data.frame()
}

sc_ar <- sc_soccog %>% 
  select(SID, Date, lonely, lonely_z) %>% 
  # mutate(lonely_z = ifelse(is.na(lonely_z) & !is.na(lonely), 0, lonely_z)) %>%
  drop_na() %>%
  group_by(SID) %>%
  nest() %>%
  ungroup() %>%
...
m <- ctModel(
  LAMBDA=diag(1), 
  type = 'stanct', #Could specify 'dt' here for discrete time.
  manifestNames='lonely_z', #Observed measurements of the latent processes
  latentNames='lonely_z', #Names here simply make parameters and plots more interpretable
  DRIFT='beta_lonely_z', #normally self feedback (diagonal drift terms) are restricted to negative
  MANIFESTMEANS=0, #For identification CINT is normally zero with this freely estimated
  time='time',
  id='SID')
m$pars$indvarying[which(m$pars$param %in% c("beta_lonely_z"))] <- TRUE
# ctModelLatex(m)
...
# plot(lonely_ct)
ctStanDiscretePars(lonely_ct, plot = T, subjects = 1:20)

lonely_ct_rx <- ctStanDiscretePars(
  lonely_ct
  , plot = F
  , subjects = 1:length(unique(sc_ar$SID))
  , times = 3
)

lonely_ct_rx <- lonely_ct_rx %>% 
  data.frame %>%
  rownames_to_column("sample") %>%
  pivot_longer(
    cols = -sample
    , names_to = c("SID", "lag", "var", "var2")
    , names_sep = "[.]"
    , values_to = "ar"
  ) %>%
  group_by(SID) %>%
  summarize(
    Estimate = median(ar)
    , lower = quantile(ar, probs = .025)
    , upper = quantile(ar, probs = .975)
    ) %>%
  ungroup() %>%
  mutate(SID = as.numeric(str_remove_all(SID, "X"))) %>%
  arrange(SID) %>%
  mutate(SID = unique(sc_ar$SID))

# lonely_ct_rx %>% select(SID, ct = Estimate) %>%
#   inner_join(lonely_ar_rx %>% select(SID, dt = Estimate)) %>%
#   select(-SID) %>%
#   cor(.)
...
sc_soccog <- sc_soccog %>%
  ...
  mutate(lonely_ar = ifelse(is.na(lonely_ar), 1, lonely_ar))
...
bl_demo_trim %>%
  select(SID, loneliness_gmc, gender_gmc, age_c, edu_years_gmc, income_log_gmc)
```
  ...
  left_join(
    ...
    bl_demo_trim %>%
      select(
        ...
        , relstat_single_gmc, relstat_sep_gmc
        )
  ) %>%
  crossing(
    RQ = c("RQ1", "RQ2")
    , cov = c("all", "none", "location")
    ) %>%
  drop_na(cog_value) %>%
...
ema_desc_tab <- sc_ema_r %>%
  kable(
    ...
    , align = c("l", rep("c", 11))
    , caption = "<strong>Table X</strong><br><em>Within-Person Descriptive Statistics, Correlations, and Reliability for all EMA variables</em>"
  ) %>%
  kable_classic(html_font = "Times") %>%
  add_header_above(c(" " = 1, "Within-Person Correlations [M (SD)]" = 8, "Descriptives" = 3))
ema_desc_tab
save_kable(ema_desc_tab, file = "03-results/04-tables/tab-x-wp-desc.html")
```
...
cog_r_tab <- cog_domains %>% 
  ...
  kable( 
    ...
    , align = c("l", rep("c", 9))
    , caption = "<strong>Table X</strong><br><em>Between-Person Descriptive Statistics and Correlations of Cognitive Tasks</em>"
  ) %>%
  kable_classic(html_font = "Times") %>%
  add_header_above(c(" " = 1, " " = 1, "Correlations" = 8))
cog_r_tab
save_kable(cog_r_tab, file = "03-results/04-tables/tab-sx-bp-cog-desc.html")
```
...
bl_r_tab
save_kable(bl_r_tab, file = "03-results/04-tables/tab-x-bp-desc.html")
```
...
bl_demo %>% 
  ...
  kable(
    ...
    , "html"
    , digits = 2
    , caption = "<strong>Table SX</strong><br><em>Attrition Analyses for Continuous Variables</em>"
  ) %>%
...
bl_demo %>% 
  ...
  kable(
    ...
    , "html"
    , digits = 2
    , caption = "<strong>Table SX</strong><br><em>Attrition Analyses for Categorical Variables</em>"
  ) %>%
  kable_classic(full_width = F, html_font = "Times") %>%
