...
# The outcomes were getting lost here, so I've split up the joining logic.
to_add_to_mi_df = ema_df |> select(SID, BeepBoops, DateTime, Day, Hour, Session, DailySessionNumber, DailyStartTime, HoursRelativeToNoon, Interrupt,Nback_score:Dsm_medianRTc)


ema_df_mi = amelia(ema_df_mi, m = 1, ts = "BeepBoops", cs = "SID", bounds = bounds_matrix)$imputations[[1]] %>%
  as_tibble() %>%
  full_join(to_add_to_mi_df, by = c("SID", "BeepBoops")) %>%
  select(-BeepBoops)

#re-order the columns
ema_df_mi = ema_df_mi |> select(SID,DateTime:Interrupt,A_cmpn:N_emoVol,Nback_score:Dsm_medianRTc) 
...
baseline_to_join = baseline_to_join[!duplicated(baseline_to_join$SID), ]
baseline_to_join 
