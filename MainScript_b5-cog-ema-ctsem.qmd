...
ema_df = ema_df |>   rename_with(~ str_replace(.x, "^.", ~ toupper(.x))) |> 
  ...
  select(SID,Time,DateTime,Day,Hour,Session,DailySessionNumber,BeepBoops,StartDate,DailyStartTime,HoursRelativeToNoon,Interrupt,
                      everything()) 
#remove duplicates
```

### Remove duplicates and filter for minimum observations
```{r}
#before min obs
ema_df = ema_df[!duplicated(ema_df), ] #check it out, non tidyverse that's more succinct and readable than tidyverse's version!

#filter for minimum 25 observations
ema_df <- ema_df %>%
  group_by(SID) %>%
  filter(n() >= 20) %>%
  ungroup()
...
ema_df_mi = amelia(ema_df_mi, m = 1, ts = "BeepBoops", cs = "SID", bounds = bounds_matrix)$imputations[[1]] %>%
  as_tibble() %>%
  full_join(ema_df %>% select(SID, BeepBoops, DateTime, Day, Hour, Session, DailySessionNumber, DailyStartTime, HoursRelativeToNoon, Interrupt), by = c("SID", "BeepBoops")) %>%
...
#reorder columns
ema_df = ema_df_mi %>%
  select(SID, DateTime, Day, Hour, Session, DailySessionNumber, DailyStartTime, HoursRelativeToNoon, Interrupt,
         A_cmpn:N_emoVol)
```
...
#relevant vars; there are also duplicates in baseline, same var values tho so we good
...
baseline_to_join = baseline_append[!duplicated(baseline_append$SID), ]
baseline_to_join 
...
test <- ema_df %>%
  left_join(baseline_to_join, by = "SID")
...
#im so sorry emorie
# => i've attempted to save our souls with the version of the code below. I created a function to that calculates these omegas, and then the code map-applies that over the names of the facets
```
