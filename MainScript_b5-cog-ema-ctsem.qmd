...
```{r}
ema_df = read_csv("../DataFromEmorie/b5-cog-ema-2025-05-07.csv")  #evan
ema_df = read_csv("~/Downloads/Projects/Cog States Continuous Time/Data/b5-cog-ema-2025-05-07.csv") #colin
...
```{r}
baseline_df = read_csv("../DataFromEmorie/b5-cog-baseline-2025-05-07.csv") #evan
baseline_df = read_csv("~/Downloads/Projects/Cog States Continuous Time/Data/b5-cog-baseline-2025-05-07.csv") #colin
...
ema_df = ema_df |> 
  ...
  rename(DateTime=Date) |> #Second, make surethe column more informative, and make sue 
  ...
  ungroup() |> 
  # now we ensure consistent first-letter capitalization
  rename_with(~ str_replace(.x, "^.", ~ toupper(.x))) |> 
  #now we reorder the columns
  select(SID,DateTime,Day,Hour,Session,DailySessionNumber,BeepBoops,StartDate,DailyStartTime,SecondsRelativeToNoon,Interrupt,
                      everything()) 
ema_df

#duplicates removed
ema_df <- ema_df[!duplicated(ema_df), ]
...
There are some no variance variables within person
```{r, warning = FALSE}
