...
-   [x] Add Time Invariant Predictors
-   [x] And Time Dependent Predictors
-   [x] need to merge with baseline data
...
df = df |> mutate(
  ...
  mutate(
    Nback_medianRTc_log = log(Nback_medianRTc + 1),
    Vpa_medianRTc_log   = log(Vpa_medianRTc + 1),
    Dsm_medianRTc_log   = log(Dsm_medianRTc + 1)
  )
```
...
```{r}
# Center time-dependent predictors within person
df <- df %>%
  group_by(SID) %>%
  mutate(
    Session_c = Session - median(Session, na.rm = TRUE),   # practice effects
    Hour_c    = HoursRelativeToNoon,                       # already centered at noon
    Interrupt_c = Interrupt                                # binary, already 0/1
  ) %>%
  ungroup()

# Center time-invariant predictors
df <- df %>%
  mutate(
    BaselineExtraversion_c    = BaselineExtraversion - mean(BaselineExtraversion, na.rm = TRUE),
    BaselineAgreeableness_c   = BaselineAgreeableness - mean(BaselineAgreeableness, na.rm = TRUE),
    BaselineConscientiousness_c = BaselineConscientiousness - mean(BaselineConscientiousness, na.rm = TRUE),
    BaselineNeuroticism_c     = BaselineNeuroticism - mean(BaselineNeuroticism, na.rm = TRUE),
    BaselineOpenness_c        = BaselineOpenness - mean(BaselineOpenness, na.rm = TRUE),
    demo_age_c                = demo_age - mean(demo_age, na.rm = TRUE)              # midpoint centering
  )
