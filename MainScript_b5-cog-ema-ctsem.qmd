...
ema_df = ema_df |> 
  ...
  mutate(DailySessionNumber = dense_rank(Hour),
         ...
         SecondsRelativeToNoon=ConvertToHoursRelativeToNoon(DateTime)) |> #note that Emorie said something about "decimal time" so maybe we should divide by 10 or 100 here?
  ungroup()
...
baseline_append = baseline_append[!duplicated(baseline_append$SID), ]

# Find SIDs in ema_df that are not in baseline_df
# setdiff(ema_df$SID, baseline_append$SID)

test <- ema_df %>%
  left_join(baseline_append, by = "SID")
...
```{r}
#I've keep the warnings unsuprressed just so we can see what happens here
