...
# Housekeeping
```{r and stuff}
#Notes on potentially speeding up CTSEM Fit()

#BEFORE DOING ANY OF THIS, Colin, we should compare the results from the test model to see if the code still works. 

#1:

#recompile cmdstanr via: 
library(cmdstanr)

# install / rebuild cmdstan with Apple-silicon flags and threading
install_cmdstan(
  release_url = "https://github.com/stan-dev/cmdstan/releases/latest/download/cmdstan-latest.tar.gz",
  cores  = parallel::detectCores(),
  cpp_options = c(
    "STAN_THREADS=true",           # enable within-chain threading
    "TBB_CXX_TYPE=clang",          # use the Home-brew clang toolchain
    "CXXFLAGS=-O3 -mcpu=apple-m3 -march=native" #O3 is aggressive optimzation
  )
)
#2:in the command line (not R), run: 
brew install llvm libomp tbb

#3. Create a file telling the computer to use specific optimizations;
##3a: in the command line check the following: 
ls -a ~ | grep .R           # do you already have an .R folder?
ls -a ~/.R | grep Makevars  # and does it already contain Makevars?
mkdir -p ~/.R
nano ~/.R/Makevars 
then paste the following: 

   ##  ~/.R/Makevars  —  per-user compiler settings ##
  CXX17      = /opt/homebrew/opt/llvm/bin/clang++ -std=c++17 -fopenmp
  CXX17FLAGS = -O3 -mcpu=apple-m3 -march=native
  LDFLAGS    = -L/opt/homebrew/opt/libomp/lib -lomp
  CPPFLAGS   = -I/opt/homebrew/opt/libomp/include


readLines("~/.R/Makevars")
Sys.getenv("HOME")        # sanity check
Sys.getenv("R_MAKEVARS_USER")  # path R actually read
system("clang++ --version")    # confirm you are using Homebrew clang
# quick test compilation
pkgbuild::has_compiler()  


## reinstall the following packages: 
#pkgs <- c("StanHeaders", "rstan", "ctsem")
...
ema_df |>   arrange(SID,Date) |> 
  ...
  #|> #First, ensure that the dataframe is sorted properly (needed for the dense_rank functions)
...
Simple Test
```{r}
#fwiw, the commas before the next line is really not my preferred coding style
#especially with the mix of new lines and non new lines. 
...
model_fit = ctStanFit(datalong = df, 
                      ctstanmodel = model,
                      optimize = TRUE,
                      priors = TRUE,
                      chains=3,
                      cores=availableCores()-1, #upping the cores but leaving a little for wiggleroom. 
                      cmdstanr_args = list(threads_per_chain = 4),
                      inits = c(0.32807, 0.73589, -3.42449, -0.50261, 0.00308, 0.00017, 0, -0.07148, -0.00191, 0.00025, -0.81974, 0.01789, 0.03066, -0.0252, 0.46397, -0.16131, 0.04506, 3.89112, -0.54048, -0.2799, -2.62945, -0.71474, -1.73866, -0.32385, -3.01588, 0.02813, 5.38677, 3.76211, 0.04559, 0.12709, 0.04942, -0.06563, 0.00072, 0.00887, -2.39033, -0.83562, -0.18136, -1.0651, 0.15507, 1.54778, -0.4617, -0.15004, -2.13088, -0.41025, 2.73894, -0.07696, 0.23611, 0.01569, 0.05742, 0.41116, 3.15461, 1.18339, 0.12245, -0.02602, 0.01359, 0.04042, 0.08907, 0.07848, 0.03027, -1.64902, -0.00877, -1.98419, -2.03337, -0.20959, -0.02728, 0.02719, 0.03587, 0.08916, 0.36252, 2e-04, -0.00801, 0.0091, 4.63846, 0.01049, 0.01064, 0.0029, 0.00299, 0.00475, -0.00722, -0.00039, -0.00025, -0.00315, -0.00784, 0.01177, 0.00439, 0.00442, 0.00501, -0.00774, 0.00802, -0.00468, 0.01552, 0.0135, -0.02208, 0.00112, -0.00437, -0.01023, -0.00047, -0.011, -0.00526, -0.00466, 0.00525, -0.0011, 0.00635, 0.01185, -0.01367, -0.0035, -0.001, -0.00151, 0.00759)
                      )
...
df
#God that's slow. I'm going to see if recompiling cmdstan with certain flags speeds things up:
```
...
install_cmdstan(
  ...
  cpp_options = c(
    "STAN_THREADS=true",           # enable within-chain threading
    "TBB_CXX_TYPE=clang",          # use the Home-brew clang toolchain
    "CXXFLAGS=-O3 -mcpu=apple-m3 -march=native"
  )
)
```
